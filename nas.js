Jsonrpc = 'http://10.10.10.10:6800/jsonrpc'; //Aria2jsonrpc地址
JsonrpcKey = 'rrsyycm'; //Aria2jsonrpc密钥
...
var Nas = {
  ...
  'VideoDown': function(request, response) {
    //获取匹配的资源
    var VideoDowInfo = JSON.parse(request.post)['url'];
    console.log(VideoDowInfo);
    var VideoName = JSON.parse(VideoDowInfo)['name'];
    var VideoUrl = JSON.parse(VideoDowInfo)['url'];
    ...
    page.open(VideoUrl, function() {
      page.includeJs("https://cdn.bootcss.com/jquery/3.4.1/jquery.js", function() {
        var list = page.evaluate(function() {
          ...
          var VideoInfoList = {};
          $("#xiazai").children(".md").children(".bluebd").children("ul").children("li").each(function(index) {
            // name: name + $(this).children("a")[0].innerHTML + '.' + $(this).children("a")[0].href.replace(/.+\./, ""),
            var item = {
              name: decodeURI($(this).children("a")[0].href.replace(/.+\//, "")),
              url: $(this).children("a")[0].href
            }
            VideoInfoList[item.name] = item;
          });
          if (Object.keys(VideoInfoList).length === 0) {
            return false;
          } else {
  ...
  'PornHub': function(request, response) {
    // Porn.settings.loadImages = false;
    // Porn.settings.diskcache = true;
    // Porn.settings.resourceTimeout = 3000;
    var PornHubInfo = JSON.parse(request.post)
    console.log(PornHubInfo.url);
    console.log('START');
    page.open(PornHubInfo.url, function(status) {
      if (status !== 'success') {
        ...
        console.log('OK');
        var videolist = page.evaluate(function() {
          var VideoName = document.title.replace(/Pornhub\.com/, "");
          var videoType = {};
          Object.keys(window).forEach(function(item) {
            if (item.startsWith('quality_')) {
              var obj = {
                name: VideoName + item.replace(/quality_/, "") + '.' + window[item].match(/.+\.(.*)\?/)[1],
                url: window[item]
              }
              videoType[obj.name] = obj;
            }
          });
          return videoType;
        });
        ...
        response.statusCode = 200;
        response.write(JSON.stringify(videolist));
        response.close();
        return
      }
    });
  ...
  'PornHubDown': function(request, response) {
    response.statusCode = 200;
    response.write(JSON.stringify({
      "info": "ok",
      "Down": JSON.parse(request.post)['url']
    }));
    response.close();
  },
  'Down': function(request, response) {
    var url = Jsonrpc;
    var DownInfo = JSON.parse(request.post)['url'];
    var DownItem = JSON.parse(DownInfo);
    var namedir = {};
    var json_rpc = {
      id: 'qwer',
      jsonrpc: '2.0',
    ...
    if (DownItem.name) {
      namedir["out"] = DownItem.name;
    }
    if (DownItem.dir) {
      namedir["dir"] = "'" + DownItem.dir + "'";
    }
    json_rpc.params.push(namedir)
    ...
    page.open(url, settings, function(status) {
      ...
      response.write(JSON.stringify({
        "info": "DownFun",
        "msg": "Down-ok:" + (DownItem.name ? DownItem.name : DownItem.url)
      }));
      // + DownItem.name ? DownItem.name : DownItem.url
      response.close();
    });
    return
  },
  ...
  'Music': function(request, response) {
    MusicName = JSON.parse(request.post)['url'];
    var url = 'https://music.jeeas.cn/v1/search?s=' + encodeURI(MusicName) + 'from=music';
    console.log(url);
    var MusicPage = require('webpage').create();
    MusicPage.open(url, function(status) {
      console.log(status);
      if (status !== 'success') {
        console.log('Unable to post!');
        response.statusCode = 200;
        ...
        // console.log('namessss' + MusicName);
        var MusicList = MusicPage.evaluate(function() {
          // console.log('0----------' + MusicName);
          return document.body.innerHTML;
        });
        var list = {};
        JSON.parse(MusicList).result.songs.forEach(function(item) {
          var info = {
            "id": item.id,
            "name": item.name
          }
  ...
  'MusicDown': function(request, response) {
    //获取音乐下载链接
    var MusicDownInfo = JSON.parse(request.post)['url'];
    var MusicId = JSON.parse(MusicDownInfo)['id'];
    var MusicName = JSON.parse(MusicDownInfo)['name'];
    var url = 'https://music.jeeas.cn/v1/music/url?id=' + MusicId + '&from=music';
    var MusicDown = require('webpage').create();
    MusicDown.open(url, function(status) {
      if (status !== 'success') {
        console.log('Unable to post!');
        response.statusCode = 200;
        response.write(JSON.stringify({
        ...
        var MusicUrl = MusicDown.evaluate(function() {
          return document.body.innerHTML;
        });
        MusicUrl = JSON.parse(MusicUrl)['data']
        response.statusCode = 200;
        // response.write(MusicUrl[0]['url']);
        response.write(JSON.stringify({
          "info": "Down",
          "Down": {
            "name": MusicName + '.' + MusicUrl[0]['url'].replace(/.+\./, ""),
            "url": MusicUrl[0]['url']
          }
        }));
        response.close();
